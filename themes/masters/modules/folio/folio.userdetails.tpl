<!-- BEGIN: MAIN -->
	<h4><!-- IF {PHP.usr.id} == {PHP.urr.user_id} AND {RPD_ADDPRD_SHOWBUTTON} --><div class="pull-xs-right"><a href="{PRD_ADDPRD_URL}" class="btn btn-success">{PHP.L.folio_add_work}</a></div><!-- ENDIF -->{PHP.L.folio}</h4>
	
	
	<!-- IF {PAGENAV_COUNT} > 0 -->	
	<ul class="nav nav-inline">
		<a class="nav-link" href="{PHP.urr.user_id|cot_url('users', 'm=details&id=$this&tab=portfolio')}">{PHP.L.All}</a>
	  	<!-- BEGIN: CAT_ROW -->
	...
	<!-- ELSE -->
	<div class="text-muted">{PHP.L.folio_empty}</div>
	<!-- ENDIF -->
