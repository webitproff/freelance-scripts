...
<form action="{SBRADD_FORM_SEND}" enctype="multipart/form-data" method="post" name="sbrform" id="sbrform">
	...
	<div class="well">
		<table class="table customform">
			<tr>
				...
				<td class="width70"><input type="text" class="form-control" name="rsbrtitle" value="{SBR_PROJECT_SHORTTITLE}"/></td>
			</tr>
			<tr>
	...
	<div id="sbrstageslist">
		...
		<div class="stageblock<!-- IF {STAGEADD_FORM_NUM} == {PHP.stagescount} --> laststage<!-- ENDIF --> well">
			<div class="removestage pull-right<!-- IF {STAGEADD_FORM_NUM} == 1 --> sr-only<!-- ENDIF -->"><i class="icon icon-remove" onclick="SbrRemoveStage(this); return false;"></i></div>
...
<script>
	...
	function StageFileAdd(obj)
	{
		...
		$(obj).parent().children('.fileslist').append('<li>\n\
			...
			<label class="file"><input type="file" name="rstagefiles[' + stagenum + '][]" /><span class="file-custom"></span></label>\n\
		</li>');
	}
