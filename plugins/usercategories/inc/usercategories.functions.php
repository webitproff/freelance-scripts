	...
	foreach ($children as $row)
	{
		if(cot_auth('usercategories', $row, $userrights)){
			...
			$t1->assign(array(
				...
				"CAT_ROW_SUBCAT" => ($subcats !== null && (count($subcats) > 0)) ? cot_usercategories_treecheck($chosen, $name, $row, $template, $userrights, $level) : '',
			));
...
function cot_usercategories_tree($chosen = '', $parent = '', $template = '', $level = 0)
{
	...
	foreach ($children as $row)
	{
		...
		$t1->assign(array(
			...
			"CAT_ROW_SUBCAT" => ($subcats !== null && (count($subcats) > 0)) ? cot_usercategories_tree($chosen, $row, $template, $level) : '',
