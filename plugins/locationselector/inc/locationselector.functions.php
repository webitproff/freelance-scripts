...
function cot_load_location()
{
	...
	if (!$cot_lf_regions || !$cot_lf_cities || !$cot_lf_locations)
	{
		...
		$where_filter = ($countriesfilter !== null && (count($countriesfilter) > 0)) ? "WHERE region_country IN ('" . implode("','", $countriesfilter) . "')" : "";
		...
		$where_filter = ($countriesfilter !== null && (count($countriesfilter) > 0)) ? "WHERE city_country IN ('" . implode("','", $countriesfilter) . "')" : "";
