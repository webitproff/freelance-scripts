...
function initialize() {
 ...
 function adrGeocoge(LatLng)
 {
  ...
  geocoder.geocode({'location': latlng}, function(results, status) {
    if (status === google.maps.GeocoderStatus.OK) {
       ...
       $('[name="ritemprdmap"]').val(adress+'#'+latlngStr[0]+','+latlngStr[1]);
 ...
 {
  geocoder.geocode( {'address': $('#locselectcountry option:selected').text() + " " + city+" "+adr}, function(results, status) {
    if (status == google.maps.GeocoderStatus.OK) {
