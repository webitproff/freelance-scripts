...
{
	global $cfg, $db, $db_projects, $db_market, $db_services, $db_folio;
	$duplicate = false;
	do
	{
		...
		switch ($aliasfor) {
			...
			case 'market':				
				...
				break;
			case 'services':				
				$queryToDB = "SELECT COUNT(*) FROM $db_services WHERE item_alias = '$alias' AND item_id != $id";
				break;
			case 'folio':				
		...
		else
		{
			switch ($aliasfor) {
				...
				case 'market':
					...
					break;
				case 'services':
					$db->update($db_services, array('item_alias' => $alias), "item_id = $id");
					break;					
				case 'folio':
