...
if ($pid > 0)
{
	$sql = $db->query("SELECT m.*, u.* FROM $db_services AS m LEFT JOIN $db_users AS u ON u.user_id=m.item_userid WHERE item_id=".$pid." LIMIT 1");
}
...
cot_block(($item['item_cost'] > 0 || $cfg['plugin']['orderservices']['acceptzerocostorders']) && $item['item_state'] == 0);

/* === Hook === */
$extp = cot_getextplugins('orderservices.neworder.first');
foreach ($extp as $pl)
{
	include $pl;
}
...
if ($a == 'add')
{
	...
	/* === Hook === */
	foreach (cot_getextplugins('orderservices.neworder.add.first') as $pl)
	{
		include $pl;
	}
	/* ===== */
	...
	/* === Hook === */
	foreach (cot_getextplugins('orderservices.neworder.add.import') as $pl)
	{
		include $pl;
	}
	/* ===== */

	//cot_check(empty($rorder['order_count']), 'orderservices_neworder_error_count', 'rcount');
	...
	/* === Hook === */
	foreach (cot_getextplugins('orderservices.neworder.add.error') as $pl)
	{
		include $pl;
	}
	/* ===== */
	...
	if (!cot_error_found())
	{
		...
		if ($db->insert($db_services_orders, $rorder))
		{
			...
			if($rorder['order_cost'] > 0) {
				...
				cot_payments_create_order('orderservices', $rorder['order_cost'], $options);
			} elseif($cfg['plugin']['orderservices']['acceptzerocostorders']) {
				cot_redirect(cot_url('orderservices', 'id='.$orderid.'&key='.$key, '', true));
			} else {
	...
	cot_redirect(cot_url('orderservices', 'm=neworder&pid=' . $pid, '', true));
	exit;
}
...
$mskin = cot_tplfile(array('orderservices', 'neworder', $structure['services'][$item['item_cat']]['tpl']), 'plug');

/* === Hook === */
foreach (cot_getextplugins('orderservices.neworder.main') as $pl)
{
	include $pl;
}
/* ===== */
...
$catpatharray[] = array(cot_url('services', 'id='.$pid), $item['item_title']);
$catpatharray[] = array('', $L['orderservices_neworder_title']);
...
$t->assign(cot_generate_servicestags($item, 'NEWORDER_SERV_', $cfg['services']['shorttextlen'], $usr['isadmin'], $cfg['homebreadcrumb']));

$t->assign(array(
	"NEWORDER_FORM_SEND" => cot_url('orderservices', 'm=neworder&pid='.$pid.'&a=add'),
...
/* === Hook === */
foreach (cot_getextplugins('orderservices.neworder.tags') as $pl)
{
	include $pl;
}
/* ===== */
